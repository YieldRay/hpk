...
describe("createProxyMiddleware", () => {
    it("proxies a GET request and returns the upstream body", async () => {
        ...
        try {
            const { status, body } = await fetch({ host: "127.0.0.1", port: proxyPort, path: "/" });
    ...
    it("sets content-length: 0 for DELETE when not provided", async () => {
        ...
        try {
            await fetch({ host: "127.0.0.1", port: proxyPort, path: "/", method: "DELETE" });
    ...
    it("strips base path before forwarding to upstream", async () => {
        ...
        try {
            await fetch({ host: "127.0.0.1", port: proxyPort, path: "/api/users?page=2" });
    ...
    it("removes restriction headers when removeRestrictionHeaders is true", async () => {
        ...
        try {
            const { headers } = await fetch({ host: "127.0.0.1", port: proxyPort, path: "/" });
    ...
    it("applies rewriteResponseOptions callback", async () => {
        ...
        try {
            const { status } = await fetch({ host: "127.0.0.1", port: proxyPort, path: "/" });
    ...
    it("calls onError when upstream is unreachable", async () => {
        ...
        try {
            await new Promise<void>((resolve) => {
                ...
                setTimeout(() => { clearInterval(interval); clientReq.destroy(); resolve(); }, 2000);
            });
